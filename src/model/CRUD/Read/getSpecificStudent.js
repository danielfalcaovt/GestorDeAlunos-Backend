...
const getSpecificStudent = async (req, res) => {
  try {
    ...
    if (
      !studentInfos.studentCPF &&
      !studentInfos.studentFirstName &&
      studentInfos.studentModule === 'Desconhecido' &&
      !studentInfos.studentParent
    ) {
      return res
        .status(404)
    ...
    const checkIfStudentAlreadyExist = await query(
      `SELECT * FROM students WHERE ${params}`,
      passedParams
    )
